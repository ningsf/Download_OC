...
- (void)downLoader:(NSURL *)url {
    ...
    //     return
    if ([FileTool fileExists:self.downLoadedPath]) {
        // UNDO: 告诉外界, 已经下载完成;
        //        NSLog(@"已经下载完成");
        self.state = DownLoadStatePauseSuccess;
        return;
    }
    ...
    // 2.2 不存在: 从0字节开始请求资源
    //     return
    if (![FileTool fileExists:self.downLoadingPath]) {
        // 从0字节开始请求资源
        [self downLoadWithURL:url offset:0];
        return;
    }
    
    _tmpSize = [FileTool fileSize:self.downLoadingPath];
...
- (void)pauseCurrentTask {
    if (self.state == DownLoadStateDownLoading) {
...
-(void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSHTTPURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler {
    ...
    if (_tmpSize == _totalSize) {
        // 1. 移动到下载完成文件夹
        //        NSLog(@"移动文件到下载完成");
        [FileTool moveFile:self.downLoadingPath toPath:self.downLoadedPath];
        // 2. 取消本次请求
...
- (void)setState:(DownLoadState)state {
    ...
    if (_state == DownLoadStatePauseSuccess && self.successBlock) {
        self.successBlock(self.downLoadedPath);
    }
    
    if (_state == DownLoadStatePauseFailed && self.faildBlock) {
        self.faildBlock();
    }
